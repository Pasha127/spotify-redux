

const MyNavbar = ()=>{
return(<>
      <div className="container navLimiter p-0 m-0 side position-sticky ">
                        <div className="logo col-10">
                            <a href="./home-page.html">{/* <img className="full" src="data:image/png;base64," alt="the logo"> */}</a>
                        </div>
                        <div className="container col-12 d-flex py-1">
                            <a href="./home-page.html"><svg xmlns="http://www.w3.org/2000/svg"  width="40" height="40" fill="white" className="bi bi-house-door-fill p-1" viewBox="0 0 16 16">
                                <path d="M6.5 14.5v-3.505c0-.245.25-.495.5-.495h2c.25 0 .5.25.5.5v3.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.146-.354L13 5.793V2.5a.5.5 0 0 0-.5-.5h-1a.5.5 0 0 0-.5.5v1.293L8.354 1.146a.5.5 0 0 0-.708 0l-6 6A.5.5 0 0 0 1.5 7.5v7a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5z"/>
                              </svg></a>
                              <span ><strong> <a href="home-page.html">Home</a> </strong></span>
                        </div>
                        <div className="container  col-12 d-flex py-1 searchBtn">
                            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="white" className="bi bi-search p-1" viewBox="0 0 16 16">
                                <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/>
                              </svg>
                              <span> <strong>Search</strong></span>
                        </div >
                        <div className="px-2">
                            <div className="container  col-12 py-1 px-1 searchFieldContainer flex-row d-none">
                                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="white" className="bi bi-search p-1" viewBox="0 0 16 16">
                                    <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/>
                                  </svg>
                                {/* <input className="searchField"> */}

                            </div>
                        </div>
                        <div className="container col-12 d-flex py-1">
                            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="white" className="bi bi-collection p-1" viewBox="0 0 16 16">
                                <path d="M2.5 3.5a.5.5 0 0 1 0-1h11a.5.5 0 0 1 0 1h-11zm2-2a.5.5 0 0 1 0-1h7a.5.5 0 0 1 0 1h-7zM0 13a1.5 1.5 0 0 0 1.5 1.5h13A1.5 1.5 0 0 0 16 13V6a1.5 1.5 0 0 0-1.5-1.5h-13A1.5 1.5 0 0 0 0 6v7zm1.5.5A.5.5 0 0 1 1 13V6a.5.5 0 0 1 .5-.5h13a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-.5.5h-13z"/>
                              </svg>
                              <span><strong>Your Library</strong></span>
                        </div>
                       
                        <div className="container col-12 d-flex py-1 mt-4">
                            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" className="bi bi-plus bg-white" viewBox="0 0 16 16">
                                <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
                              </svg>
                              <span><strong>Create Playlist</strong></span>
                        </div>
                        <div className="container col-12 d-flex py-1">
                            <p className="heart d-flex">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="white" className="bi bi-heart-fill align-self-center " viewBox="0 0 16 16">
                                <path fillRule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z"/>
                              </svg>
                            </p>
                              <span className=""><strong>Liked Songs</strong></span>
                        </div>
                        <div className="line"></div>
                        <div className="container songs mt-4">
                            <p className="d-block sideRec" >Funky Heavy Bluesy</p>
                            <p className="d-block sideRec" >Your Top Songs 2020</p>
                            <p className="d-block sideRec" >In Love With You </p>
                            <p className="d-block sideRec" >This Is Rod Stewart</p>
                            <p className="d-block sideRec" > Young Thug </p>                            
                            <p className="d-block sideRec" >Palleggio</p>
                            <p className="d-block sideRec" >June</p>
                            <p className="d-block sideRec" >Halloween</p>
                            <p className="d-block sideRec" >Birthday Party</p>
                            <p className="d-block sideRec" >Metal Ballads</p>
                            <p className="d-block sideRec" >Spice Girls</p>
                            <p className="d-block sideRec" >Jazz Classics</p>
                            <p className="d-block sideRec" >Bach</p>
                            <p className="d-block sideRec" >Handel</p>
                            <p className="d-block sideRec" >The Hu</p>
                            <p className="d-block sideRec" >The Beatles</p>
                            <p className="d-block sideRec" >Chill</p>
                            <p className="d-block sideRec" >Breakup</p>
                            <p className="d-block sideRec" >Love</p>
                            <p className="d-block sideRec" >The Wiggles</p>
                            <p className="d-block sideRec" >Blue Man Group</p>
                            <p className="d-block sideRec" >Rock</p>
                            <p className="d-block sideRec" >Synth</p>
                            <p className="d-block sideRec" >Funk</p>
                            <p className="d-block sideRec" >Mellow</p>
                            <p className="d-block sideRec" >Retro</p>
                            <p className="d-block sideRec" >Party</p>
                           
                        </div>
                        <div className="container col-12 d-flex py-1">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="white" className="bi bi-arrow-down-circle" viewBox="0 0 16 16">
                                <path fillRule="evenodd" d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8zm15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8.5 4.5a.5.5 0 0 0-1 0v5.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V4.5z"/>
                              </svg>
                              <a href="https://play.google.com/store/apps/details?id=com.spotify.music&hl=en_US&gl=US" className=""><strong>Install App</strong></a>
                        </div>
            
                        
                    </div>
</>)
    }

    export default MyNavbar